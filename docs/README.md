...
├── 开发计划/    # 按主题分类的计划与设计方案（索引见其 README）
...
| [上游成熟适配吸收计划](开发计划/基础与规划/ESP32-P4上游成熟适配吸收计划.md) | 上游 P4 基线的选择性同步边界、步骤和回归矩阵 | 计划同步 Apache NuttX / OpenVela P4 改动时 |
| [ESP-IDF LCD 参考实现对比与收敛计划](开发计划/显示与触摸/ESP32-P4X-ESP-IDF-LCD参考实现对比与收敛计划.md) | 与同硬件可显示的 ESP-IDF 工程逐项对照 DPI、GDMA、Bridge 与视觉验收 | P4X 背光亮但 DSI 色条黑屏时 |
