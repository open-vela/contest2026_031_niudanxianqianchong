...
| 芯片控制器 | `chips/esp32p4/common/espressif/esp_hosted_sdio.c`、`chips/esp32p4/include/esp_hosted_sdio.h` | GPIO Matrix 信号路由、SDMMC 时钟/复位、同步命令原语、错误快照 |
| 芯片私有传输 | `chips/esp32p4/common/espressif/esp_hosted_transport.c`、`chips/esp32p4/include/esp_hosted_transport.h` | CMD0/CMD5/CMD3/CMD7/CMD52 枚举和响应状态检查 |
...
用户完成固件编译、烧录及本次板端运行；本次提交整理没有重新编译固件。整理只调整注释、帮助文本和文档，不改变已实测的运行逻辑。开发过程中曾通过临时主机桩测试：16 个枚举/错误场景和 8 个命令处理场景；这些不是实板稳定性证据，也不是仓库内持久化回归测试。

## 验证边界和下一步

- 已确认：一次启动完成 CMD0 → CMD5 → CMD3 → CMD7 → CMD52，读取 CCCR=0x32。
- 待确认：10 次 RESET、5 次断电重启，以及失败后重新初始化的实板恢复能力；当前不能宣称稳定性验收通过。
- 未验证：CMake 完整构建、4-bit/高速模式、CMD53、数据 DMA、Function 1 中断、ESP-Hosted 版本握手和 Wi-Fi 联网。接口存在不等于这些能力可用。
- C6 固件来源、版本及构建配置尚未确认。CCCR 读取成功不能证明 ESP-Hosted 协议兼容。

下一阶段按以下顺序推进，每个阶段独立验证后再提交：

1. 固定 C6 固件基线，补充启动稳定性记录。
2. 在传输层读取能力、启用 Function 1、等待就绪并配置/回读块大小；保留当前低速单线基线。
3. 在控制器层实现 CMD53 数据传输、超时与错误回收，在传输层实现从机寄存器/FIFO 访问；先验收有限长度收发，再扩展 DMA 和中断队列。
4. 对齐 C6 固件的 ESP-Hosted 包格式和版本握手，验证连续控制包收发及复位恢复。
5. 实现 NuttX 网络接口和 Wi-Fi 控制映射，注册 `wlan0`，依次验收关联、DHCP、DNS，再接入 Smart Home 云端调用。
